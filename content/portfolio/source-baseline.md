# Devina Azaria — Portfolio Source Baseline

Source: sanitized Curriculum Vitae, last updated August 2026.

This file is an editorial inventory for devina.id. Evidence from the legacy `/profilku/` site will be matched before individual records are treated as verified portfolio evidence.

## Identity

Devina Azaria — student at SMA Negeri 3 Yogyakarta, Class X (2026).

## Portfolio direction

The portfolio preserves a multidisciplinary learning history across academics, science competitions, music, language, Javanese literature and culture, communication, and school activities, while documenting the developing direction toward science, engineering, and aerospace.

## Education

- SMA Negeri 3 Yogyakarta — 2026–, Class X
- SMP Negeri 5 Yogyakarta — 2023–2026
- SD Mutiara Persada Yogyakarta International School — 2017–2023
- TK Mutiara Persada Yogyakarta — 2015–2017

## Achievement categories

### Academic & International Competitions

The August 2026 CV records national-level science and English competition results during 2024–2025, including silver and bronze awards and a Jenius Science Olympiad science silver result in 2025.

### Language, Literature & Culture

The CV records competition participation and awards from 2021–2025 across geguritan, macapat, Javanese script, and Mandarin storytelling, ranging from finalist placements to first-place awards at city, provincial, interregional, and national competition contexts.

Verified or matched public records so far:

- 2025 — Juara II Macapat Remaja, Kompetisi Bahasa dan Sastra Kota Yogyakarta. Primary public evidence: Devina's Instagram post showing the award placard; official event context: Pemerintah Kota Yogyakarta / Dinas Kebudayaan Kota Yogyakarta. Status: `VERIFIED` and published.
- 2025 — Juara III Maca Geguritan Remaja, Kompetisi Bahasa dan Sastra Kota Yogyakarta. Primary public evidence: Devina's Instagram post showing the award placard; official event context: Pemerintah Kota Yogyakarta / Dinas Kebudayaan Kota Yogyakarta. Status: `VERIFIED` and published.
- 2024 — Juara 1 Lomba Macapat, Semarak Bulan Bahasa MAN 1 Sleman; represented SMP Negeri 5 Yogyakarta. Public source: Kementerian Agama Kabupaten Sleman. Status: `VERIFIED` and published.
- 2024 — Juara 2 Maca Geguritan, Lomba Seni dan Budaya SMA Negeri 9 Yogyakarta; represented SMP Negeri 5 Yogyakarta. Public source: SMA Negeri 9 Yogyakarta. Status: `VERIFIED` and published.
- 2024 — Juara Favorit Macapat, Lomba Seni dan Budaya SMA Negeri 9 Yogyakarta; represented SMP Negeri 5 Yogyakarta. Public source: SMA Negeri 9 Yogyakarta. Status: `VERIFIED` and published.

### Music & Performing Arts

The CV records band competition results in 2024–2025, including Best Drummer at Prambanan Sound Scape in 2025 and several band placements.

## Skills recorded in source

Languages: Bahasa Indonesia, Bahasa Inggris, Mandarin, Bahasa Jawa.

Music: drums, guitar, piano, violin.

Communication: public speaking, storytelling, performance, presentation.

## Activities & interests recorded in source

- Astronomy and science competitions
- Mathematics competitions
- Music and band
- Javanese literature and culture
- Mandarin storytelling
- Public speaking
- Creative projects

## Non-formal learning themes

The source records long-running learning experiences in music, English, macapat, classical Javanese dance, public speaking and media production, digital literacy, Javanese script, voice work, acting, and Cambridge-based schooling.

## Evidence model

Each public portfolio record should eventually contain:

- title
- year or date
- category
- level
- organizer
- result or role
- factual description
- evidence status
- original photo or certificate when available
- Instagram or YouTube evidence when useful
- provenance note

Evidence states:

- `UNAUDITED` — present in source but evidence not yet matched
- `MATCHED` — supporting evidence located
- `VERIFIED` — evidence and record details checked
- `PUBLIC` — approved for publication on devina.id

## Site mapping

- Academic and science achievements → `/achievements/`
- Language and literature → `/achievements/` and `/culture/`
- Music awards and performances → `/music/` and `/achievements/`
- Javanese cultural activity → `/culture/`
- School and community activity → `/activities/`
- Historical learning material → `/journal/`
- Engineering and scientific work → `/projects/` and `/research/`

## Status

Sanitized CV source accepted as the portfolio baseline. Five culture/literature records from 2024–2025 are now published on `/achievements/`: three matched to external institutional sources and two 2025 results verified from primary public evidence with official event context. Evidence matching against the remaining legacy WordPress `/profilku/` inventory remains pending.